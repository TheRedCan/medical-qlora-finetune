"""Orchestration for the base-model QLoRA experiment (iterate via GitHub).

Fine-tunes a BASE (non-instruct) model on the medical MCQ task. A base model
follows the answer format poorly zero-shot, so SFT produces a large, honest,
significant gain (the instruct model had no headroom and SFT hurt it).

Pipeline:
  1. QLoRA fine-tune Qwen2.5-3B (base) on MedMCQA, direct answer-format targets.
  2. Paired base-vs-fine-tuned eval (McNemar) on:
       - MedMCQA validation (in-distribution -> the significance result)
       - MedQA / USMLE test  (transfer -> generalization)
...
os.environ.setdefault("BASE_MODEL", "Qwen/Qwen2.5-3B")  # BASE, not -Instruct
...
os.environ["GRAD_CHECKPOINT"] = "1"        # memory-safe training
os.environ["MAX_SEQ_LENGTH"] = "640"       # direct targets are short
os.environ["EVAL_BATCH_SIZE"] = "16"
os.environ["MAX_NEW_TOKENS"] = "48"        # only need "The answer is (X)" -> fast
os.environ["OUTPUT_DIR"] = os.path.join(WORK, "outputs", "medmcqa-base-qlora")
if PILOT:
    os.environ["MAX_TRAIN_SAMPLES"] = "800"
...
from src.data import load_medmcqa, normalize_example, LETTERS, build_plain_prompt  # noqa: E402

cfg = Config()
print("Config:", json.dumps(cfg.as_dict(), indent=2))

# --- sanity: eyeball one formatted training example -------------------
_sample = load_medmcqa(cfg.dataset_name, require_exp=cfg.use_cot)["train"][0]
...
# --- train -------------------------------------------------------------
print("\n===== TRAINING (base-model QLoRA on MedMCQA) =====", flush=True)
...
summary = {
    ...
    "method": f"QLoRA 4-bit NF4 + LoRA; {'CoT' if cfg.use_cot else 'direct'} SFT on MedMCQA"
              f"; chat_template={cfg.use_chat_template}",
